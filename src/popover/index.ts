...
import { EventListenerBinder, IPopover, LabeledElement, alignment, alwanApp, alwanConfig, side } from "../types";
import { getBounds, getInteractiveElements, translate } from "../utils/dom";
import { isNumber, isString, isset } from "../utils/is";
import { abs, round } from "../utils/math";
import { toArray } from "../utils/object";
import { CENTER, END, GAP, START, fallbackAlignments, fallbackSides } from "./constants";
...
export const createPopover = (
    ...
    const setPosition = () => {
        ...
        const viewportRect = getBounds(ROOT);
        const targetRect = getBounds(target);
        const popoverRect = getBounds(popoverElement);
        const offsetParentRect = getBounds(getOffsetParent(popoverElement), true);
        ...
        translate(
            ...
            ...(<[x: number, y: number]>coordinates.map((value, axis) => {
                ...
                // the viewport height.
                if (axis && value === null && popoverRect[3] > viewportRect[5]) {
                    popoverStyleDeclaration.height = viewportRect[5] - GAP * 2 + 'px';
                ...
                return round(
                    ( isset(value)
                        ...
                        : (viewportRect[axis + 4] - popoverRect[axis + 2]) / 2
    ...
    const handleKeyboard = (e: Event) => {
        if (_isOpen()) {
            ...
            if (key === ESCAPE) {
                ...
                const hasTabbedAwayFromPopover = (shiftKey && target === firstFocusableElement) ||
    ...
    */
    const handleBackdropClick = ({ target }: Event) => {
        if (
            ...
            !toArray((<LabeledElement>alwanReferenceElement).labels || []).some((label) =>
